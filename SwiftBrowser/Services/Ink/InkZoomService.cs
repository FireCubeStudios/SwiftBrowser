...
namespace flowpad.Services.Ink
{
    public class InkZoomService
    {
        ...
        public float ZoomIn(float zoomFactor = DefaultZoomFactor)
        {
            return ExecuteZoom(_scrollViewer.ZoomFactor + zoomFactor);
        }


        public float ZoomOut(float zoomFactor = DefaultZoomFactor)
        {
            return ExecuteZoom(_scrollViewer.ZoomFactor - zoomFactor);
        }
        ...
        {
            if (_scrollViewer.Content is FrameworkElement element) FitToSize(element.Width, element.Height);
        }
